...
class Dots:
    ...
    # BEGIN CODE CHANGES
    def eat(self, x, y, position):  # You might want/need to pass arguments here.
        if position == "top" and self.top_row:
            index = []
            for i in range(0, len(self.top_row)):
                dot = self.top_row[i]
            ...
            for i in index:
                del self.top_row[i]
        if position == "bottom" and self.bottom_row:
            index = []
            for i in range(0, len(self.bottom_row)):
                dot = self.bottom_row[i]
                if abs(x - dot.x) <= self.EAT_DIST:
                    index.append(i)
            for i in index:
                del self.bottom_row[i]
        if position == "left" and self.left_col:
            index = []
            for i in range(0, len(self.left_col)):
                dot = self.left_col[i]
            ...
            for i in index:
                del self.left_col[i]
        if position == "right" and self.right_col:
            index = []
            for i in range(0, len(self.right_col)):
                dot = self.right_col[i]
                if abs(y - dot.y) <= self.EAT_DIST:
                    index.append(i)
            for i in index:
